...
try:
    ...
    def select_container():
        try:
            cur = con.cursor()
            sql = "SELECT CNTR_NO FROM ship_schedule.container WHERE STATUS_DT between (SELECT date_format(now(), '%Y-%m-%d') FROM dual) AND (SELECT date_format(DATE_ADD(now(), INTERVAL 5 DAY), '%Y-%m-%d') FROM dual);"
            print(sql)
            cur.execute(sql)
            # con.close()
            rows = cur.fetchall()
