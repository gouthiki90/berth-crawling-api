...
try:
    ...
    def upsert_container(STATUS_DT, STATUS_NM, CNTR_STATUS, STATUS_TM, container_status):
        try:
            cur = con.cursor()
            sql = f"INSERT INTO hang_man.container (STATUS_DT, STATUS_NM, CNTR_STATUS, STATUS_TM, container_status) VALUES ('{STATUS_DT}', '{STATUS_NM}', '{CNTR_STATUS}', '{STATUS_TM}', {container_status}) ON DUPLICATE KEY UPDATE STATUS_DT = '{STATUS_DT}', STATUS_NM = '{STATUS_NM}', CNTR_STATUS = '{CNTR_STATUS}', STATUS_TM = '{STATUS_TM}', container_status = {container_status}"
            print(sql)
            cur.execute(sql)
            # con.close()
            rows = cur.fetchall()
